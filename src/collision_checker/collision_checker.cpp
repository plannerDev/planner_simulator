...
fcl::Transform3f eigenToFcl(const Eigen::Isometry3d& tf) {
    ...
    fcl::Vec3f fclTranslation(tf.translation().x(), tf.translation().y(),
                              tf.translation().z());
...
bool CollisionChecker::checkShapeGroupCollision(
    ...
    const std::pair<std::string,
                    std::vector<std::shared_ptr<fcl::CollisionObject>>>& g2) {
...
void CollisionChecker::constructFCLObjectRobotLink(
    ...
    for (std::size_t j = 0; j < shapes.size(); j++) {
        std::shared_ptr<fcl::CollisionGeometry> g =
...
void CollisionChecker::constructFCLObjectWorld(
    ...
    for (const auto& objectId : world->getObjectIds()) {
        ...
        for (std::size_t i = 0; i < object->shapes_.size(); i++) {
            ...
            fcl::CollisionObject fcl_obj(g, eigenToFcl(tf));
            auto fcl_objectPtr =
                std::make_shared<fcl::CollisionObject>(fcl_obj);
            fclObjects.push_back(fcl_objectPtr);
        }
        ...
        case shapes::MESH: {
            ...
            if (mesh->vertex_count > 0 && mesh->triangle_count > 0) {
                ...
                for (unsigned int i = 0; i < mesh->vertex_count; ++i)
                    points[i] = fcl::Vec3f(mesh->vertices[3 * i],
                                           mesh->vertices[3 * i + 1],
                                           mesh->vertices[3 * i + 2]);
