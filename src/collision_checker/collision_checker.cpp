...
namespace gsmpl {
namespace {
// Eigen::Isometry3d fclToEigen(const fcl::Transform3d& fclTf) {
...
fcl::Transform3d eigenToFcl(const Eigen::Isometry3d& tf) {
    fcl::Transform3d fclTf;
    // fcl::Vector3d fclTranslation(tf.translation().x(), tf.translation().y(),
    //                           tf.translation().z());
...
bool CollisionChecker::robotSelfCollisionCheck() {
    ...
    for (const auto& link1 : fclObjsRobotMap_) {
        for (const auto& link2 : fclObjsRobotMap_) {
            bool found = acm_.getAllowedCollision(link1.first, link2.first,
                                                  allowedCollision);
            if (found) {
                if (allowedCollision ==
                    collision_detection::AllowedCollision::Type::NEVER) {
...
bool CollisionChecker::checkShapeGroupCollision(
    ...
    const std::pair<std::string,
                    std::vector<std::shared_ptr<fcl::CollisionObjectd>>>& g2) {
    std::vector<std::shared_ptr<fcl::CollisionObjectd>> g1Shapes = g1.second;
    std::vector<std::shared_ptr<fcl::CollisionObjectd>> g2Shapes = g2.second;
    for (const auto& g1Shape : g1Shapes) {
        for (const auto& g2Shape : g2Shapes) {
            fcl::CollisionRequestd request;
            request.enable_contact = true;
            fcl::CollisionResultd result;
            fcl::CollisionObjectd* o1 = g1Shape.get();
            fcl::CollisionObjectd* o2 = g2Shape.get();
...
// {
//     std::vector<std::shared_ptr<fcl::CollisionObjectd>> objs =
//     fclObjsRobotMap_["panda_hand"];

//     if (objs.size() >= 1)
//     {
//         std::shared_ptr<fcl::CollisionObjectd> obj = objs[0];
//         fcl::Transform3d fcl_tf = obj->getTransform();
...
void CollisionChecker::constructFCLObjectRobotLink(
    const std::string name, const Eigen::Isometry3d& tf) {
    std::vector<std::shared_ptr<fcl::CollisionObjectd>> fclObjects;
    ...
    for (std::size_t j = 0; j < shapes.size(); j++) {
        std::shared_ptr<fcl::CollisionGeometryd> g =
            fclGeometriesRobot_[name][j];
        fcl::CollisionObjectd fcl_obj(g, eigenToFcl(tf));
        auto fcl_objectPtr = std::make_shared<fcl::CollisionObjectd>(fcl_obj);
...
void CollisionChecker::constructFCLGeometryRobot() {
    ...
    for (const auto& name : linkNames_) {
        fclGeometriesRobot_.insert(
            std::make_pair(name, constructFCLGeometryRobotLink(name)));
...
CollisionChecker::constructFCLGeometryRobotLink(const std::string& name) {
    ...
    std::vector<std::shared_ptr<fcl::CollisionGeometryd>> geometries;

    for (std::size_t i = 0; i < shapes.size(); i++) {
        std::shared_ptr<fcl::CollisionGeometryd> g =
            constructFCLGeometry(shapes[i]);
...
void CollisionChecker::constructFCLObjectWorld(
    ...
    for (const auto& objectId : world->getObjectIds()) {
        ...
        for (std::size_t i = 0; i < object->shapes_.size(); i++) {
            std::shared_ptr<fcl::CollisionGeometryd> g =
                constructFCLGeometry(object->shapes_[i]);
            ...
            fcl::CollisionObjectd fcl_obj(g, eigenToFcl(tf));
            auto fcl_objectPtr =
                std::make_shared<fcl::CollisionObjectd>(fcl_obj);
            fclObjects.push_back(fcl_objectPtr);
        }
...
std::shared_ptr<fcl::CollisionGeometryd> CollisionChecker::constructFCLGeometry(
    const shapes::ShapeConstPtr& shape) {
    // handle cases individually
    switch (shape->type) {
        case shapes::PLANE: {
            const shapes::Plane* p =
                static_cast<const shapes::Plane*>(shape.get());
            return std::make_shared<fcl::Planed>(p->a, p->b, p->c, p->d);
        }
        case shapes::SPHERE: {
            const shapes::Sphere* s =
                static_cast<const shapes::Sphere*>(shape.get());
            return std::make_shared<fcl::Sphered>(s->radius);
        }
        case shapes::BOX: {
        ...
        case shapes::CYLINDER: {
            ...
            return std::make_shared<fcl::Cylinderd>(s->radius, s->length);
        }
        case shapes::CONE: {
            const shapes::Cone* s =
                static_cast<const shapes::Cone*>(shape.get());
            return std::make_shared<fcl::Coned>(s->radius, s->length);
        }

        case shapes::MESH: {
            auto g = std::make_shared<fcl::BVHModel<fcl::OBBRSSd>>();
            const shapes::Mesh* mesh =
                static_cast<const shapes::Mesh*>(shape.get());
            if (mesh->vertex_count > 0 && mesh->triangle_count > 0) {
                std::vector<fcl::Triangle> triIndices(mesh->triangle_count);
                for (unsigned int i = 0; i < mesh->triangle_count; ++i)
                    triIndices[i] = fcl::Triangle(mesh->triangles[3 * i],
                                                  mesh->triangles[3 * i + 1],
                                                  mesh->triangles[3 * i + 2]);

                std::vector<fcl::Vector3d> points(mesh->vertex_count);
                for (unsigned int i = 0; i < mesh->vertex_count; ++i)
                    points[i] = fcl::Vector3d(mesh->vertices[3 * i],
                                           mesh->vertices[3 * i + 1],
                                           mesh->vertices[3 * i + 2]);
        ...
        case shapes::OCTREE: {
            const shapes::OcTree* g =
                static_cast<const shapes::OcTree*>(shape.get());
            return std::make_shared<fcl::OcTreed>(g->octree);
        }
        default:
            LOGD("This shape type (%d) is not supported using FCL yet",
                 (int)shape->type);
            return nullptr;
    }
}
