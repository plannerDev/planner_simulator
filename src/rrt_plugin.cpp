...
public:
    ...
    bool initialize(const moveit::core::RobotModelConstPtr& model,
        ...
        for (const std::string& gpName : model->getJointModelGroupNames()) {
            LOGD("group name: %s, robot model: %s", gpName.data(),
                 model->getName().data());
            std::cout << "group name: " << gpName.data()
                      << " robot model: " << model->getName().data()
                      << std::endl;
            planningContexts_[gpName] =
                RRTPlanningContextPtr(new RRTPlanningContext(
                    "rrt_planning_context", gpName, model, node));
        }
        return true;
    }
    ...
    planning_interface::PlanningContextPtr getPlanningContext(
        ...
        const RRTPlanningContextPtr& context =
            planningContexts_.at(req.group_name);
...
PLUGINLIB_EXPORT_CLASS(rrt_interface::RRTPlannerManager,
