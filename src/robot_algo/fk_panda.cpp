...
namespace gsmpl {
FKPanda::FKPanda(moveit::core::RobotModelConstPtr robotModel, const std::string group)
    : robotModel_(robotModel), group_(group)
{
    ...
    for (const auto& name : activeLinkNames_) {
        ...
        //     }
        // }
        activeJointTypes_.push_back(JointType::Revolute); // TODO: add other joint type case
...
void FKPanda::lookupLinktf(const State& q, const Eigen::Isometry3d& tfPre, std::size_t index)
{
    ...
    if (index < (q.size() - 1))
        ...
        // both panda_leftfinger & panda_rightfinger's have a parent link panda_hand
    }
}
Eigen::MatrixXd FKPanda::jacobian(const State& q)
{
    ...
    for (std::size_t jointIndex = 0; jointIndex < activeLinkNames_.size(); jointIndex++) {
        Eigen::Isometry3d jointTransform = linksTf_[activeLinkNames_[jointIndex]];
    ...
    // From "Quaternion kinematics for the error-state KF" 4.5.1 Global-to-local relations
    // dq/dt = 0.5 * wg ⊗ q
    ...
    out.block(3, 0, 4, columns) = 0.5 * quatUpdateMatrix * out.block(3, 0, 3, columns);
    return out;
}
...
Eigen::VectorXd se3ToVector(const Eigen::Isometry3d& tf)
{
    ...
    quat.normalize();
    Eigen::VectorXd v(7);
    v << translation.x(), translation.y(), translation.z(), quat.w(), quat.x(), quat.y(), quat.z();
    return v;
}
...
void FKPanda::Test_Jacobian(const State& q)
{
    ...
    //     std::cout << "Failed to compute J_global eigenvalues and eigenvectors!" << std::endl;
    // Eigen::VectorXcd eigenvalues = solver.eigenvalues();
    // std::cout << "J_global Eigenvalues:" << std::endl << eigenvalues << std::endl << std::endl;
    ...
    //     std::cout << "Failed to compute J2 eigenvalues and eigenvectors!" << std::endl;
    // Eigen::VectorXcd eigenvalues2 = solver2.eigenvalues();
    // std::cout << "J_Estimate Eigenvalues:" << std::endl << eigenvalues2 << std::endl << std::endl;
    std::cout << "J_Estimate - J_global" << std::endl;
    std::cout << J_Estimate - J_global << std::endl;
    std::cout << "J_Estimate - J_global norm: " << (J_Estimate - J_global).norm() << std::endl;

    std::cout << "J_Estimate - J_local" << std::endl;
    std::cout << J_Estimate - J_local << std::endl;
    std::cout << "J_Estimate - J_local norm: " << (J_Estimate - J_local).norm() << std::endl;
}
} // namespace gsmpl
